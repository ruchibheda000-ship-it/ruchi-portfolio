...
export default function Home() {
  ...
  // Physical 3D Cover Angle and Inner Page Reveal calculations
  // 0% -> Closed (angle 0deg)
  // 100% -> Fully Open (angle 180deg)
  const coverAngle = Math.min(180, scrollProgress * 180)
  const isCoverLifting = scrollProgress > 0.02
  const isOpenFlat = scrollProgress >= 0.95
  ...
  return (
    <div className="min-h-screen bg-[#061840] text-[#F1E3CC] font-body selection:bg-[#E4BA83] selection:text-[#061840] relative overflow-x-hidden pb-24">
      ...
      {/* INITIAL STATE (0%): CLOSED BLUE DIARY with "Portfolio" (Reference 1 & 2)  */}
      {/* SCROLLING (0% -> 100%): COVER PHYSICALLY HINGES OPEN AROUND CENTRAL SPINE  */}
      {/* FULLY OPEN (100%): TWO-PAGE OPEN SPREAD (PAGE 1 INTRO, PAGE 2 3 BELIEFS)   */}
      {/* ========================================================================= */}
      <div ref={scrollTrackRef} className="relative w-full h-[230vh]">
        
        {/* Sticky Viewport Container */}
        <div className="sticky top-6 sm:top-10 w-full min-h-[92vh] flex flex-col items-center justify-center pointer-events-auto px-3 sm:px-6">
          ...
          <div className="w-full max-w-4xl lg:max-w-5xl flex items-center justify-between mb-2 sm:mb-3 font-mono text-[10px] sm:text-xs text-[#8DA1B4]">
            <div className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${isOpenFlat ? 'bg-[#E4BA83]' : 'bg-[#0B3272] border border-[#E4BA83]/60'}`}></span>
              <span className="uppercase tracking-wider">
                {isOpenFlat ? 'PHYSICAL DIARY // FULLY OPENED' : scrollProgress > 0.1 ? `OPENING SPREAD // ${Math.round(scrollProgress * 100)}%` : 'PERSONAL NOTEBOOK // CLOSED COVER'}
              </span>
            </div>
            <button
            ...
            >
              <span>{isOpenFlat ? '▲ Close Diary' : '▼ Scroll / Click to Open'}</span>
            </button>
          </div>

          {/* THE PHYSICAL DIARY (3D Perspective Spine Container) */}
          <div className="relative z-20 w-full max-w-4xl lg:max-w-5xl [perspective:2400px]">
            
            {/* SILK BOOKMARK RIBBON (Gold #E4BA83 silk ribbon peeking from spine) */}
            ...
            {/* ------------------------------------------------------------------- */}
            {/* PHYSICAL DIARY CONTAINER                                            */}
            {/* When closed: shows the Blue Cover with "Portfolio"                  */}
            {/* When opening: Cover physically lifts upward around central spine    */}
            {/* When open: Upper Page 1 & Lower Page 2 form a balanced landscape     */}
            {/* ------------------------------------------------------------------- */}
            <div className="relative w-full book-ground-shadow rounded-[32px] sm:rounded-[44px] overflow-visible">
              
              {/* UPPER HALF: BLUE COVER + PAGE 1 (Hinges open upwards around spine) */}
              <div
                className="relative rounded-t-[32px] sm:rounded-t-[44px] bg-crumpled-cover border-paper-cover border-b-0 p-2.5 sm:p-4 pb-0 origin-bottom [transform-style:preserve-3d] transition-shadow duration-300"
                style={{
                  transform: `rotateX(${-Math.max(0, (1 - scrollProgress) * 75)}deg)`,
                  filter: `brightness(${0.88 + scrollProgress * 0.12})`
                }}
              >
                {/* PAGE 1 INTERIOR (Warm Cream Paper with Blue Grid - Equal Height to Page 2) */}
                <div className="relative rounded-t-[24px] sm:rounded-t-[36px] bg-diary-grid text-[#061840] p-6 sm:p-10 lg:p-12 border border-[#E8DEC8] border-b-0 page-upper-depth overflow-hidden min-h-[460px] sm:min-h-[500px] lg:min-h-[520px] flex flex-col justify-center">
              ...
              {/* LOWER HALF: BLUE COVER + PAGE 2 (Hinges open downwards around spine) */}
              <div
                className="relative rounded-b-[32px] sm:rounded-b-[44px] bg-crumpled-cover border-paper-cover border-t-0 p-2.5 sm:p-4 pt-0 origin-top [transform-style:preserve-3d] transition-shadow duration-300"
                style={{
                  transform: `rotateX(${Math.max(0, (1 - scrollProgress) * 55)}deg)`,
                  filter: `brightness(${0.9 + scrollProgress * 0.1})`
                }}
              >
                {/* PAGE 2 INTERIOR (Warm Paper - Equal Height to Page 1, Exactly matching Image 2) */}
                <div id="diary-beliefs" className="relative rounded-b-[24px] sm:rounded-b-[36px] bg-[#F6E8D2] px-6 sm:px-12 lg:px-16 pt-8 sm:pt-10 pb-6 sm:pb-8 border border-[#E8DEC8] border-t-0 page-lower-depth text-[#061840] overflow-hidden min-h-[460px] sm:min-h-[500px] lg:min-h-[520px] flex flex-col justify-between">
              ...
              {/* ------------------------------------------------------------------- */}
              {/* THE CLOSED FRONT COVER OVERLAY (Visible at 0% scroll)               */}
              {/* Features the exact hybrid "Portfolio" typography from Reference 1   */}
              {/* Physically hinges upward around the spine as the user scrolls       */}
              {/* ------------------------------------------------------------------- */}
              <div
                onClick={toggleDiaryOpen}
                className="absolute inset-0 z-50 rounded-[32px] sm:rounded-[44px] bg-crumpled-cover border-paper-cover p-6 sm:p-12 flex flex-col justify-between origin-bottom [transform-style:preserve-3d] cursor-pointer select-none transition-opacity duration-200"
                style={{
              ...
              >
                {/* Visible paper block edge peeking under the cover */}
                <div className="absolute -bottom-2.5 right-6 left-6 h-2.5 rounded-b-md closed-book-paper-block opacity-90 pointer-events-none"></div>
                ...
                {/* THE HERO TYPOGRAPHY: "Portfolio" (Directly inspired by Reference 1) */}
                <div className="my-auto py-6">
                  <PortfolioCoverWord />
                </div>

                {/* Cover Footer / Scroll CTA */}
                <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-[#F1E3CC]/15 text-xs font-mono text-[#8DA1B4]">
