      ...
      <main className="relative z-10 w-full max-w-3xl mx-auto px-4 sm:px-6 py-8 flex-grow flex items-center justify-center">
        ...
        <div className="w-full rounded-sm p-4 sm:p-6 bg-[#c28e57] shadow-2xl border-4 sm:border-8 border-[#996531] ring-1 ring-black/20">
          <div className="rounded-sm bg-cork p-4 sm:p-8 overflow-hidden shadow-inner border border-[#804e22]">
            ...
            <div className="relative bg-[#FFFED8] bg-notebook-grid text-[#173C64] p-6 sm:p-10 rounded-xs shadow-xl rotate-[0.5deg] border-2 border-[#173C64] paper-crease">
              ...
              <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-[#173C64] mb-2">
                Send a Message
              </h1>
              <p className="text-xs sm:text-sm text-[#173C64]/80 mb-6">
                Have a project in mind, an inquiry about packaging or identity, or just want to connect? Leave a note below.
              </p>

              {submitted ? (
                <div className="bg-[#FFFED8] border-2 border-dashed border-[#173C64] p-8 rounded-sm text-center shadow-inner">
                  <div className="flex justify-center mb-3">
                    <PixelSparkle size={32} className="text-[#173C64]" />
                  </div>
                  <h3 className="font-display font-extrabold text-xl text-[#173C64] mb-1">
                    Message Dispatched!
                  </h3>
                  <p className="text-xs sm:text-sm text-[#173C64]/80 mb-4">
                    Thank you, {formData.name || 'friend'}. I'll respond as soon as possible.
                  </p>
                  <button
                    onClick={() => setSubmitted(false)}
                    className="font-display font-bold text-xs bg-[#173C64] text-[#FFFED8] px-4 py-2 rounded-full hover:bg-[#0e2640] transition-colors"
                  >
                    Send Another Note
                  </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider mb-1 text-[#173C64]">
                      Your Name
                    </label>
                    <input
                      type="text"
                      ...
                      className="w-full bg-white/80 border border-[#173C64] rounded-sm px-3.5 py-2 text-sm text-[#173C64] placeholder-[#173C64]/40 focus:outline-none focus:ring-2 focus:ring-[#173C64]"
                    />
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 pt-2">
                    ...
                    <button
                      type="submit"
                      className="font-display font-extrabold text-sm bg-[#173C64] text-[#FFFED8] px-6 py-2.5 rounded-full hover:bg-[#0e2640] shadow-sm transition-all hover:scale-102 active:scale-98"
                    >
                      Dispatch Note →
                    </button>
                  </div>
